...
- 钱包内部按分记账，但**任何界面不得出现人民币**：金额一律经展示货币层（后端 `services/billing/money.py`，用户端 `lib/money.ts`，管理端 `lib/currency.ts`）格式化为 VND / USD。充值仅支持银行转账 + 管理员确认到账。详见 [BILLING.md](./BILLING.md)。
