...
/** 充值订单；pay_type 为 bank_transfer（银行转账）或历史渠道，pay_amount/pay_currency 是下单时记录的应付展示金额 */
...
// 管理员确认银行转账已到账（仅 pending 单），可附备注
export function confirmAdminOrder(outTradeNo: string, note?: string): Promise<AdminOrder> {
  const trimmed = note?.trim();
  return api<AdminOrder>(`/api/admin/orders/${encodeURIComponent(outTradeNo)}/confirm`, {
    method: "POST",
...
// 管理员关闭待支付订单
export function closeAdminOrder(outTradeNo: string): Promise<AdminOrder> {
  return api<AdminOrder>(`/api/admin/orders/${encodeURIComponent(outTradeNo)}/close`, {
    method: "POST",
  });
}
...
export type AdminModelSettings = {
  ...
  cdn_base: string;
  billing_display_currency: "VND" | "USD";
  billing_cny_vnd: number;
  billing_usd_cny: number;
  topup_bank_name: string;
  topup_bank_account: string;
  topup_bank_holder: string;
  topup_bank_bin: string;
  topup_order_expire_hours: number;
...
export type AdminTemplate = {
  ...
  is_active: boolean;
  is_premium: boolean;
  /** 模板名称/描述的 en、vi 译文（缺失时回落中文主值） */
  i18n?: Partial<Record<"en" | "vi", { name?: string; description?: string }>> | null;
};
