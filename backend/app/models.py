...
class Template(Base):
    ...
    # 界面展示用多语言名称/描述：{"en": {"name", "description"}, "vi": {...}}；name/description 仍为中文主值
...
class Order(Base):
    ...
    pay_type: Mapped[str] = mapped_column(String(16))  # bank_transfer（历史：alipay | wxpay）
    ...
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
    # 用户实际需支付的展示金额与币种（下单时快照；汇率变动不影响已建订单）
    pay_amount: Mapped[int | None] = mapped_column(Integer, nullable=True)
    pay_currency: Mapped[str | None] = mapped_column(String(8), nullable=True)
    # 管理员确认 / 关闭时的备注
