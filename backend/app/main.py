...
async def _apply_schema_patches() -> None:
    ...
    async with engine.begin() as conn:
        ...
        if "content_lang_locked" not in pcols:
            await conn.execute(
                text("ALTER TABLE projects ADD COLUMN content_lang_locked BOOLEAN DEFAULT FALSE")
            )
        ...
        # 订单：展示金额 / 币种 / 管理员备注（银行转账充值）
        ocols = await _pg_columns(conn, "orders")
        if "pay_amount" not in ocols:
            await conn.execute(text("ALTER TABLE orders ADD COLUMN pay_amount INTEGER"))
        if "pay_currency" not in ocols:
            await conn.execute(text("ALTER TABLE orders ADD COLUMN pay_currency VARCHAR(8)"))
        if "note" not in ocols:
            await conn.execute(text("ALTER TABLE orders ADD COLUMN note VARCHAR(255)"))

        # 模板多语言名称/描述（界面展示用）
        tcols = await _pg_columns(conn, "templates")
        ...
        if "provider_channel_id" not in trcols:
            await conn.execute(text("ALTER TABLE task_runs ADD COLUMN provider_channel_id VARCHAR(64)"))
...
async def seed_templates() -> None:
    ...
    async with AsyncSessionLocal() as db:
        for item in TEMPLATES:
            data = dict(item)
            translations = TEMPLATE_I18N.get(data["id"]) or {}
            ...
            if existing:
                ...
                # 多语言译文只在库内为空时补齐，后台改过的不覆盖
                if translations and not (existing.i18n or {}):
