...
type KepuStepKey = 'topic' | 'style' | 'confirm' | 'assets' | 'videos' | 'compose'
const KEPU_STEP_KEYS: KepuStepKey[] = ['topic', 'style', 'confirm', 'assets', 'videos', 'compose']

// 按当前界面语言取步骤标签
function kepuStepLabel(key: KepuStepKey): string {
  const map = messages[getActiveLocale()].kepuFlow.steps as Record<string, string>
  return map[key] || key
}

/** 科普全链路步骤：建项 / 风格 / 分镜台共用（label 按当前语言惰性求值） */
export const KEPU_STEPS: Array<{ key: KepuStepKey; readonly label: string }> = KEPU_STEP_KEYS.map((key) => ({
  key,
  get label() {
    return kepuStepLabel(key)
  },
}))
...
export function kepuPhaseHint(project: {
  ...
  const hint = messages[getActiveLocale()].kepuFlow.phaseHint
  if (isRunning(effectiveStatus(project))) {
    return hint.running
  }
  const phase = kepuBillingPhase(project)
  if (phase === 'script') return hint.script
  if (phase === 'assets') return hint.assets
  if (phase === 'videos') return hint.videos
  return hint.compose
}
