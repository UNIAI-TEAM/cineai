...
export type Template = {
  ...
  is_premium: boolean
  sort_order: number
  /** 各语言的名称/描述：{ en: { name, description }, vi: {...} }，缺省回落 name/description */
...
export type BillingSkuList = {
  skus: BillingSku[]
  pay_types: string[]
  /** false 表示收款账户尚未配置，充值暂未开放 */
...
export type BillingCheckout = {
  ...
  sku_id: string
  sku_name: string
  pay_type: 'bank_transfer' | string
  amount_fen: number
  credit_fen: number
  pay_amount: number
  pay_currency: string
  bank: { name: string; account: string; holder: string; bin?: string }
  /** 转账备注（= 订单号），管理员据此核对 */
  transfer_note: string
  /** 配置了银行 BIN 时返回 VietQR 图片地址，否则为空串 */
  vietqr_url?: string
...
export const api = {
  ...
  /** 展示货币与汇率（公开接口） */
  billingCurrency() {
    return request<BillingCurrency>('/api/billing/currency')
  },
  billingSkus() {
    return request<BillingSkuList>('/api/billing/skus')
  },
  /** 创建银行转账充值单 */
  createBillingOrder(sku_id: string) {
    return request<BillingCheckout>('/api/billing/orders', {
      method: 'POST',
